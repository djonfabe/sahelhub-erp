...
class RevenueController extends Controller
{
    ...
    public function index(Request $request)
    {
        if(Auth::user()->can('manage-revenues')){
            ...
            $allowedSortFields = ['created_at', 'revenue_date', 'amount', 'reference_number', 'name'];
            if ($request->sort && in_array($request->sort, $allowedSortFields)) {
                $query->orderBy($request->sort, $request->direction === 'asc' ? 'asc' : 'desc');
            } else {
                $query->latest();
            }
