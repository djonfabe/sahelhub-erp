...
class CleanDemoData extends Command
{
    ...
    protected $description = 'Supprime toutes les données demo en gardant superadmin, plans, paramètres et templates';
    ...
    public function handle(): int
    {
        ...
        // ── 1. Vider la file d'attente pour éviter que des jobs re-créent des données ──
        $this->line('  <comment>Vidage de la file de jobs...</comment>');
        Artisan::call('queue:flush');

        DB::statement('SET FOREIGN_KEY_CHECKS=0');

        // ── 2. Tables métier core ──────────────────────────────────────────────────────
        $coreTables = [
            // Ventes
        ...
        $moduleTables = [
            ...
            'form_conversions', 'form_responses', 'forms',
            'zoom_meetings',
            'newsletter_subscribers',
            // Budget
            'budget_planner_items', 'budget_planners',
            // Double entrée
            'double_entry_ledgers', 'double_entry_transactions',
            // Formation
            'training_feedbacks', 'training_trainers', 'training_tasks', 'training_types', 'trainings',
            // Telegram / Twilio
            'telegram_bots',
        ];
        ...
        // ── 4. Utilisateurs entreprises (garder superadmin) ───────────────────────────
        $companyUserIds = DB::table('users')
            ->where('type', '!=', 'superadmin')
            ->pluck('id');

        if ($companyUserIds->isNotEmpty()) {
            // Supprimer les paramètres propres aux entreprises
            DB::table('settings')->whereIn('created_by', $companyUserIds)->delete();
            // Supprimer les modules actifs des entreprises
            if (Schema::hasTable('user_active_modules')) {
                DB::table('user_active_modules')->whereIn('user_id', $companyUserIds)->delete();
            }
        ...
        // ── 5. Vider les caches applicatifs ───────────────────────────────────────────
        $this->line('  <comment>Vidage des caches...</comment>');
