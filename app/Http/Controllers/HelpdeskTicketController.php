...
class HelpdeskTicketController extends Controller
{
    public function index()
    {
        if(Auth::user()->can('manage-helpdesk-tickets')){
            $tickets = HelpdeskTicket::query()
                ...
                ->when(request('sort') && in_array(request('sort'), ['created_at', 'title', 'status', 'priority', 'ticket_id']), fn($q) => $q->orderBy(request('sort'), request('direction') === 'desc' ? 'desc' : 'asc'), fn($q) => $q->latest())
    ...
    public function show(HelpdeskTicket $helpdeskTicket)
    {
        if(Auth::user()->can('view-helpdesk-tickets')){
            if(Auth::user()->type !== 'superadmin' && $helpdeskTicket->created_by !== creatorId()){
                return back()->with('error', __('Permission denied'));
            }
